...
| **Hold Back Spoiler IncRems** | Boolean | `true` | When a Rem is both an Incremental Rem and a flashcard, holds the IncRem back while any of its own cards is still due, so reading the extract cannot give away the answer before you grade the card. Released as soon as the card is graded, and released anyway once nothing else is due. Normal queue only — *Practice All* and *In Order* are unaffected. 📖 [Spoiler Protection](Reviewing-Items-in-the-Queue.md#spoiler-protection). |
